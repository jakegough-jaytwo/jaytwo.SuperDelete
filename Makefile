...
build: restore
	dotnet build ./jaytwo.SuperDelete.sln

test: unit-test
  
unit-test: build
	rm -rf out/testResults
	dotnet test ./test/jaytwo.SuperDelete.Tests \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.SuperDelete.Tests.trx"

pack:
	rm -rf out/packed
	cd ./src/jaytwo.SuperDelete; \
